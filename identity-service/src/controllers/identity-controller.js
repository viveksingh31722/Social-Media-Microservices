...
const registerUser = async (req, res) => {
  ...
  try {
    ...
    if (user) {
      logger.warn("User already present with email or username");
      return res
        .status(400)
...
const loginUser = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      logger.warn("user not found with the provided email");
      return res
        .status(400)
...
const refreshTokenController = async (req, res) => {
  ...
  try {
    ...
    const storedToken = await RefreshToken.findOne({ token: refreshTokens });
